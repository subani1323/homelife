...
const TradeDetailsModal = ({ trade, onClose, onUpdate }) => {
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      if (trade && trade._id) {
        try {
          const [realEstateRes, commissionRes, ledgerRes] = await Promise.all([
            axiosInstance.get(`/real-estate-trust-eft/trade/${trade._id}`),
          ...
          // Get ledger entries and filter for this trade
          const allLedgerEntries = ledgerRes.data || [];
          const tradeNumber = trade.tradeNumber || trade.keyInfo?.tradeNumber;
          ledgerEntries = allLedgerEntries.filter(
            (entry) =>
              entry.tradeNumber === tradeNumber ||
              (entry.description &&
                entry.description.includes(`Trade #: ${tradeNumber}`))
          );
        } catch (err) {
      ...
      // For AGENT DOUBLE ENDER, track transaction order per agent
      const agentDoubleEnderOrder = new Map();
      if (tradeClassification === "AGENT DOUBLE ENDER") {
        (trade.agentCommissionList || []).forEach((a, index) => {
          if (a.classification === "AGENT DOUBLE ENDER" && a.agentId) {
            const key = a.agentId;
            if (!agentDoubleEnderOrder.has(key)) {
              agentDoubleEnderOrder.set(key, []);
            }
            agentDoubleEnderOrder.get(key).push(index);
          }
        });
      }
      ...
      const agentCalculations = (trade.agentCommissionList || []).map(
        (agent, agentIndex) => {
          ...
          // When buyer rebate is included:
          // - Agent amount shown = agentAmount - feesDeducted (buyer rebate already deducted from agentAmount)
          // - HST calculated on = agentAmount + buyerRebateAmount - feesDeducted (full commission before rebate minus fees)

          const agentCommissionAfterRebate = agentAmount - agentFeesDeducted;
          const agentCommissionForHST =
            agentAmount + buyerRebateAmount - agentFeesDeducted;
          ...
          // For AGENT DOUBLE ENDER: use transactionType to determine which column
          if (
            tradeClassification === "AGENT DOUBLE ENDER" &&
            agent.classification === "AGENT DOUBLE ENDER"
          ) {
            // Determine transaction type - use saved value or infer from order
            let transactionType = agent.transactionType;

            // If transactionType is missing, infer from order
            if (!transactionType && agent.agentId) {
              const agentIndices =
                agentDoubleEnderOrder.get(agent.agentId) || [];
              const positionInList = agentIndices.indexOf(agentIndex);
              // First occurrence (position 0) = LISTING, second (position 1) = SELLING
              transactionType = positionInList === 0 ? "LISTING" : "SELLING";
            }

            if (transactionType === "LISTING") {
              // First transaction goes to Listing column
              agentListing = agentCommissionAfterRebate.toFixed(2);
              agentSelling = "0.00";
            } else if (transactionType === "SELLING") {
              // Second transaction goes to Selling column
              agentListing = "0.00";
              agentSelling = agentCommissionAfterRebate.toFixed(2);
            }
          } else if (isCoOperatingSide) {
            // For CO-OPERATING SIDE: put agent amount in selling column

            agentListing = "0.00";

            // Show agent commission (buyer rebate already deducted from agentAmount)
          ...
          // HST is calculated on the full commission amount (agentAmount + buyerRebateAmount - feesDeducted)

          const agentHST = (agentCommissionForHST * 0.13).toFixed(2);
      ...
      if (tradeClassification === "AGENT DOUBLE ENDER") {
        // For AGENT DOUBLE ENDER: Net to Office = Original Commission - Agent Commission per column
        // Listing side: Original Commission Listing - Agent Listing (from LISTING transaction)
        // Selling side: Original Commission Selling - Agent Selling (from SELLING transaction)
        // Use original commission amounts (listingAmount, sellingAmount) not Base Office Commission

        // Find the LISTING and SELLING transactions
        let listingTransaction = agentCalculations.find(
          (agent) => agent.transactionType === "LISTING"
        );
        let sellingTransaction = agentCalculations.find(
          (agent) => agent.transactionType === "SELLING"
        );

        // If transactionType is missing, infer from order (first = LISTING, second = SELLING)
        if (!listingTransaction && agentCalculations.length >= 1) {
          listingTransaction = agentCalculations[0];
        }
        if (!sellingTransaction && agentCalculations.length >= 2) {
          sellingTransaction = agentCalculations[1];
        }

        // Calculate Net to Office for Listing column
        // Use original listingAmount (not baseOfficeCommissionListing which may have outside broker deductions)
        const listingAgentAmount = parseFloat(
          listingTransaction?.agentListing || 0
        );
        netToOfficeListing = (
          parseFloat(listingAmount) - listingAgentAmount
        ).toFixed(2);

        // Calculate Net to Office for Selling column
        // Use original sellingAmount (not baseOfficeCommissionSelling which may have outside broker deductions)
        const sellingAgentAmount = parseFloat(
          sellingTransaction?.agentSelling || 0
        );
        netToOfficeSelling = (
          parseFloat(sellingAmount) - sellingAgentAmount
        ).toFixed(2);

        // Calculate Sub-Total, HST, and Total
        netToOfficeSubTotal = (
          parseFloat(netToOfficeListing) + parseFloat(netToOfficeSelling)
        ).toFixed(2);

        // HST is calculated on the Net to Office amount
        netToOfficeHST = (parseFloat(netToOfficeSubTotal) * 0.13).toFixed(2);

        netToOfficeTotal = (
          parseFloat(netToOfficeSubTotal) + parseFloat(netToOfficeHST)
        ).toFixed(2);
      } else if (!isCoOperatingSide && agentCalculations.length === 2) {
      ...
      Markham, Ontario L6E OB7<br>
        ...
        <div class="section">
          ...
          <table style="width:100%; border-collapse:collapse; margin-bottom:10px;">
            ...
            <tbody>
              ...
              ${trade.agentCommissionList
                .filter(
                  (agent) => agent.classification === "AGENT DOUBLE ENDER"
                )
                .map(
                  (agent) => `
        ...
        <div class="section">
          ...
          <div class="section-title">Commission Trust Activity</div>

          ${(() => {
            // For Listing Side, include CommissionTransfer from Real Estate Trust Payments as first transaction
            // For Co-Operating Side, include CommissionTransfer with "Commission Received" description as first transaction
            let displayCommissionTrustEFTs = [...commissionTrustEFTs];
            if (
              tradeClassification === "LISTING SIDE" ||
              tradeClassification === "Listing Side"
            ) {
              const commissionTransferEFT = realEstateTrustEFTs.find(
                (eft) => eft.type === "CommissionTransfer"
              );
              if (commissionTransferEFT) {
                displayCommissionTrustEFTs = [
                  commissionTransferEFT,
                  ...commissionTrustEFTs,
                ];
              }

              // Check if trust amount is less than total commission and add "Received From" transaction as second item
              const trustRecords = trade.trustRecords || [];
              const commissionIncome =
                trade.commission?.commissionIncomeRows?.[0] || {};
              const listingAmount = commissionIncome.listingAmount
                ? parseFloat(commissionIncome.listingAmount)
                : 0;
              const sellingAmount = commissionIncome.sellingAmount
                ? parseFloat(commissionIncome.sellingAmount)
                : 0;
              const listingTax = listingAmount * 0.13;
              const sellingTax = sellingAmount * 0.13;
              const totalCommission =
                listingAmount + sellingAmount + listingTax + sellingTax;

              // Calculate total trust amount
              const trustAmount = trustRecords.reduce((sum, record) => {
                const amount = parseFloat(
                  record.amount || record.depositAmount || record.total || 0
                );
                return sum + amount;
              }, 0);

              // If trust amount is less than total commission, find and add "Received From" transaction
              if (trustAmount < totalCommission) {
                // First, try to find the "Received From" transaction from ledger entries (account 10004 with "Received From")
                let receivedFromLedger = null;
                if (ledgerEntries && ledgerEntries.length > 0) {
                  receivedFromLedger = ledgerEntries.find(
                    (entry) =>
                      entry.accountNumber === "10004" &&
                      entry.debit > 0 &&
                      entry.description &&
                      (entry.description.includes("Received From:") ||
                        entry.description.includes("Received from:"))
                  );
                }

                if (receivedFromLedger) {
                  // Extract recipient from description (format: "Trade #: X - Received From: Recipient - Commission Trust - Address")
                  let recipient = receivedFromLedger.payee || "";
                  if (!recipient && receivedFromLedger.description) {
                    const receivedFromMatch =
                      receivedFromLedger.description.match(
                        /Received From:\s*([^-]+?)(?:\s*-|$)/i
                      );
                  ...
                  // Create the balance commission payment transaction from ledger entry
                  const balanceCommissionPayment = {
                    eftNumber:
                      receivedFromLedger.eftNumber ||
                      receivedFromLedger.reference?.replace("EFT#", "") ||
                      "",
                    date:
                      receivedFromLedger.date || receivedFromLedger.chequeDate,
                    chequeDate:
                      receivedFromLedger.chequeDate || receivedFromLedger.date,
                    amount:
                      receivedFromLedger.debit || receivedFromLedger.amount,
                    recipient: recipient || "N/A",
                    type: "BalanceCommissionPayment",
                    description: "Balance Amount Received",
                    _id: `balance-${receivedFromLedger._id || Date.now()}`,
                  };

                  // Insert as second transaction (after CommissionTransfer)
                  if (commissionTransferEFT) {
                    displayCommissionTrustEFTs = [
                      commissionTransferEFT,
                      balanceCommissionPayment,
                      ...commissionTrustEFTs,
                    ];
                  } else {
                    displayCommissionTrustEFTs = [
                  ...
                  // Fallback: Find the "Received From" transaction from trust records
                  const receivedFromRecord = trustRecords.find(
                    (record) =>
                      record.receivedFrom && record.paymentType === "EFT"
                  );

                  if (receivedFromRecord) {
                    // Extract EFT number from reference or use a default
                    const eftNumber =
                      receivedFromRecord.reference?.replace("EFT#", "") ||
                      receivedFromRecord.eftNumber ||
                      "";

                    // Create the balance commission payment transaction
                    const balanceCommissionPayment = {
                      eftNumber: eftNumber,
                      date:
                        receivedFromRecord.depositDate ||
                        receivedFromRecord.date,
                      chequeDate:
                        receivedFromRecord.depositDate ||
                        receivedFromRecord.date,
                      amount: parseFloat(
                        receivedFromRecord.amount ||
                          receivedFromRecord.depositAmount ||
                          receivedFromRecord.total ||
                          0
                      ),
                      recipient: receivedFromRecord.receivedFrom || "",
                      type: "BalanceCommissionPayment",
                      description: "Balance Amount Received",
                      _id: `balance-${receivedFromRecord._id || Date.now()}`,
                    };
              ...
              tradeClassification === "CO-OPERATING SIDE" ||
              tradeClassification === "Co-Operating Side"
            ) {
              // First, try to find CommissionTransfer in Real Estate Trust Payments
              let commissionReceivedEFT = realEstateTrustEFTs.find(
                (eft) => eft.type === "CommissionTransfer"
              );

              // If not found, look for a transaction with "Received From" in Commission Trust EFTs
              if (!commissionReceivedEFT) {
                commissionReceivedEFT = commissionTrustEFTs.find(
                  (eft) =>
                    eft.description &&
                    (eft.description.includes("Received From") ||
                      eft.description.includes("Received from") ||
                      eft.description.match(
                        /Received From:.*Commission Trust/i
                      ) ||
                      eft.description.includes("Commission Received"))
                );
              }

              // If still not found, check ledger entries from trade finalize (account 10004 with "Received From")
              if (
                !commissionReceivedEFT &&
                ledgerEntries &&
                ledgerEntries.length > 0
              ) {
                const commissionReceivedLedger = ledgerEntries.find(
                  (entry) =>
                    entry.accountNumber === "10004" &&
                    entry.debit > 0 &&
                    entry.description &&
                ...
                if (commissionReceivedLedger) {
                  // Extract recipient from description (format: "Trade #: X - Address - Received From: Recipient")
                  let recipient = commissionReceivedLedger.payee || "";
                  if (!recipient && commissionReceivedLedger.description) {
                    const receivedFromMatch =
                      commissionReceivedLedger.description.match(
                        /Received From:\s*([^-]+?)(?:\s*-|$)/i
                      );
                    if (receivedFromMatch) {
                      recipient = receivedFromMatch[1].trim();
                  ...
                  // Create EFT-like object from ledger entry
                  commissionReceivedEFT = {
                    eftNumber:
                      commissionReceivedLedger.eftNumber ||
                      commissionReceivedLedger.reference?.replace("EFT#", "") ||
                      "",
                    date:
                      commissionReceivedLedger.date ||
                      commissionReceivedLedger.chequeDate,
                    chequeDate:
                      commissionReceivedLedger.chequeDate ||
                      commissionReceivedLedger.date,
                    amount:
                      commissionReceivedLedger.debit ||
                      commissionReceivedLedger.amount,
                    recipient: recipient || "N/A",
                    type: "CommissionTransfer",
                    description: "Commission Received",
                    _id: commissionReceivedLedger._id,
                  };
                }
              }

              // If still not found, check by recipient name (common outside broker names)
              if (!commissionReceivedEFT) {
                commissionReceivedEFT = commissionTrustEFTs.find(
                  (eft) =>
                    eft.recipient &&
                    (eft.recipient.includes("HomeLife") ||
                      eft.recipient.includes("Future Realty") ||
                      eft.recipient.includes("Realty Inc"))
                );
              }

              if (commissionReceivedEFT) {
                // Create a formatted transaction with "Commission Received" as description
                const formattedEFT = {
                  ...commissionReceivedEFT,
                  description: "Commission Received",
                };
                // If the transaction was found in commissionTrustEFTs, remove it from the array first
                const remainingEFTs =
                  commissionReceivedEFT._id &&
                  commissionTrustEFTs.some(
                    (eft) => eft._id === commissionReceivedEFT._id
                  )
                    ? commissionTrustEFTs.filter(
                        (eft) => eft._id !== commissionReceivedEFT._id
                      )
                    : commissionTrustEFTs;
                displayCommissionTrustEFTs = [formattedEFT, ...remainingEFTs];
              }
            }
            return displayCommissionTrustEFTs.length > 0
              ? `<table><thead><tr>
              ...
              : `<div class="text-gray-500">No Commission Trust Activity found for this trade.</div>`;
          })()}
  ...
  const handleDropboxAuth = async () => {
    try {
      ...
      console.log(
        "\n?? If you see an error, make sure this redirect URI is added to Dropbox:"
      );
  ...
  const handleDropboxUpload = async () => {
    ...
    try {
      ...
      if (trade && trade._id) {
        try {
          const [retResponse, ctResponse, ledgerRes] = await Promise.all([
            axiosInstance.get(`/real-estate-trust-eft/trade/${trade._id}`),
            axiosInstance.get(`/commission-trust-eft/trade/${trade._id}`),
            axiosInstance.get("/ledger"),
          ]);
  ...
  const EFTPaymentTables = ({ trade }) => {
    ...
    useEffect(() => {
      ...
      // Fetch ledger entries and filter for this trade
      axiosInstance
        .get("/ledger")
        .then((res) => {
          const allLedgerEntries = res.data || [];
          const tradeNumber = trade.tradeNumber || trade.keyInfo?.tradeNumber;
          const filteredEntries = allLedgerEntries.filter(
            (entry) =>
              entry.tradeNumber === tradeNumber ||
              (entry.description &&
                entry.description.includes(`Trade #: ${tradeNumber}`))
    ...
    return (
      <div className="mb-8">
        ...
        {/* Commission Trust Activity Table */}
          ...
          {(() => {
            ...
            return displayCommissionTrustEFTs.length > 0 ? (
              <table className="min-w-full border border-gray-300 mb-2 text-sm">
                ...
                <tbody>
                  {displayCommissionTrustEFTs.map((eft) => (
                    <tr key={eft._id || eft.eftNumber}>
                      <td className="border px-2 py-1">{eft.eftNumber}</td>
              ...
              <div className="text-gray-500">
                No Commission Trust Activity found for this trade.
              </div>
            );
          })()}
        </div>
